...
# 2025.10.15 Add CellChatDBv2
#######################################
def extract_LR(species, database='CellChatDB', datahost='package'):
    """
    Extract regulatory network data for mouse or human.

    Parameters:
        species (str): 'mouse' or 'human'
        database (str): 'CellChatDB' or 'CellChatDBv2'  
    ...
    if database == 'CellChatDB':
        species_files = {'mouse': 'mouse-interaction_input_CellChatDB.csv', 'human': 'human-interaction_input_CellChatDB.csv'}
    elif database == 'CellChatDBv2':     
        species_files = {'mouse': 'mouse-CellChatDB_v2_interaction.csv', 'human': 'human-CellChatDB_v2_interaction.csv'}
    else: 
        raise ValueError(f"Database type: {database} is not supported.")
    ...
    if datahost == 'package':
        ...
        ## Download from web (only for CellChatDB, v2 not supported in original code)
        if database == 'CellChatDB':
            urls = {
            ...
            try:
                ...
                os.remove(file_path)    # Ensure file is removed even if reading fails
        else:
            raise ValueError(f"Web download for {database} is not supported.") 
    return LR_pair
