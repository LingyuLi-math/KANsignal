...
#################################################
# 2025.05.06 When run Between_spot_demo.ipynb
# The following need be muted
# 2025.05.19 Use hest env, need this part
#################################################
...
from .utils_hest import (ALIGNED_HE_FILENAME, check_arg, deprecated,
...
class HESTData:
    ...
    ######################################################################
    # 2025.09.23 
    # adjust save_spatial_plot by add 'size=None'
    # involve: HESTData.py, readers.py, SThest_Vit_AI_NPC.ipynb
    ######################################################################
    def save_spatial_plot(self, save_path: str, name: str='', key='total_counts', size=None, pl_kwargs={}):
        ...
        save_spatial_plot(self.adata, save_path, name, key, size, pl_kwargs)
...
####################################################
# 2025.05.19 move these three to processData
####################################################
####################################################################################################
# class HESTIterator:
...
def save_spatial_plot(adata: sc.AnnData, save_path: str, name: str='', key='total_counts', size=None, pl_kwargs={}):
    ...
    fig = sc.pl.spatial(adata, show=False, img_key="downscaled_fullres", color=[key], title=f"in_tissue spots: total_counts", size=size, return_fig=True, **pl_kwargs)
