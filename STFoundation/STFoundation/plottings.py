...
######################################################
# 2025.05.19
# plot spot_distribution and adata_expression
######################################################
def window_patch_expr(data, num_to_plot=16, cols=4, s_size=100, cnt_color='viridis', gene='CD27'):
    """
    Plot spatial data or windows based on input data type.
    
    Args:
    data (list or numpy.ndarray): List of AnnData objects or an array of windows to plot.
    num_to_plot (int): Number of elements or windows to plot. Defaults to 16.
    cols (int): Number of columns in the plot grid. Defaults to 4.
    cnt_color (str): Color map for the scatter plot. Defaults to 'viridis' (used only for AnnData).
    gene (str): Gene to plot (used only for AnnData).
    """
    ## Determine the type of data and select items to plot accordingly
    if isinstance(data, np.ndarray):
        # Handling numpy array for window plotting
        items_to_plot = data[:num_to_plot]
        plot_function = lambda ax, item: ax.scatter(item[:, 0], item[:, 1], alpha=0.6)
        set_title = lambda ax, i: ax.set_title(f"Window {i+1}")
    elif all(isinstance(d, type(data[0])) for d in data) and 'AnnData' in str(type(data[0])):
        # Handling list of AnnData objects
        items_to_plot = data[:num_to_plot]
        plot_function = lambda ax, item: ax.scatter(item.obsm['spatial_std'][:, 0], item.obsm['spatial_std'][:, 1],
                                                    c=item.to_df()[str(gene)], s=s_size, cmap=cnt_color)
        set_title = lambda ax, i: ax.set_title(f"Window {i+1}")
        invert_yaxis = True
    else:
        raise ValueError("Unsupported data type. Data should be either numpy.ndarray or list of AnnData objects.")

    ## Calculate the number of rows needed in the plot grid
    rows = (len(items_to_plot) + cols - 1) // cols

    fig, axs = plt.subplots(rows, cols, figsize=(cols * 3, rows * 3 / 2), dpi=200)
    axs = axs.flatten()

    for i, item in enumerate(items_to_plot):
        plot_function(axs[i], item)
        axs[i].set_aspect('equal', adjustable='box')
        set_title(axs[i], i)
        # if 'invert_yaxis' in locals() and invert_yaxis:
        axs[i].invert_yaxis()

    ## If # subgraphs is greater than the number of items, close extra subgraphs
...
## not use
def window_plotting(windows, num_windows_to_plot=16, cols=4):
    
    """Plot a specified number of windows.

    Args:
    windows (numpy.ndarray): An array of windows to plot.
    num_windows_to_plot (int): The number of windows to plot. Defaults to 16.
    cols (int): The number of columns in the plot grid. Defaults to 4.
    """
    ## Select the windows to plot
    windows_to_plot = windows[:num_windows_to_plot]
    rows = (len(windows_to_plot) + cols - 1) // cols  
    ...
    for i, window in enumerate(windows_to_plot):
        axs[i].scatter(window[:, 0], window[:, 1], alpha=0.6)  
        axs[i].set_aspect('equal', adjustable='box')   
        axs[i].set_title(f"Window {i+1}")

    ## If # subgraphs is greater than the number of windows, close extra subgraphs
    for j in range(i + 1, len(axs)):
        axs[j].axis('off')
...
## not use
def window_dataexpr(adata_list, gene='CD27', num_to_plot=16, cols=4, cnt_color='viridis'):
    """
    Plot spatial data from a list of AnnData objects.

    Args:
    adata_list (list of AnnData): List of AnnData objects to plot.
    num_to_plot (int): Number of AnnData objects to plot. Defaults to 16.
    cols (int): Number of columns in the plot grid. Defaults to 4.
    cnt_color (str): Color map for the scatter plot. Defaults to 'viridis'.
    """
    ## Select the adata objects to plot
    adata_to_plot = adata_list[:num_to_plot]

    # Calculate the number of rows needed in the plot grid
    rows = (len(adata_to_plot) + cols - 1) // cols
    ...
    for i, adata in enumerate(adata_to_plot):
        axs[i].scatter(adata.obsm['spatial_std'][:, 0], adata.obsm['spatial_std'][:, 1],
                       c=adata.to_df()[str(gene)], s=100, cmap=cnt_color)
        axs[i].set_aspect('equal', adjustable='box')
        axs[i].set_title(f"Window {i + 1}")
        axs[i].invert_yaxis()
...
###########################
import umap
def plot_umap(concat_adata, concat_labels, fig_size=(8, 6), random_state=42, dpi_value=100,
              min_dist=0.6, verbose=False, save_path=None):
    """
    Create a UMAP plot for an AnnData object with corresponding labels.

    Parameters:
    - concat_adata: AnnData object or numpy array of data to be plotted.
    - concat_labels: Array of labels corresponding to `concat_adata`.
    - fig_size: Tuple specifying the size of the figure.
    - random_state: Seed used by the random number generator.
    - dpi_value: DPI setting for the figure.
    - min_dist: The effective minimum distance between embedded points, defaults to 0.6.
    - umap_kwargs: Additional keyword arguments for UMAP.
    - verbose: Verbosity level of the UMAP algorithm.
    - save_path: Path where the plot should be saved. If None, the plot is shown.
    """
    
    # Update umap_kwargs with min_dist if not already set
    # umap_kwargs.setdefault('min_dist', min_dist)
    umap_kwargs={'min_dist':min_dist}

    # Initialize UMAP model with specified parameters and additional arguments
    umap_model = umap.UMAP(metric='euclidean', verbose=verbose, random_state=random_state, **umap_kwargs)
    embedding = umap_model.fit_transform(concat_adata)

    plt.figure(figsize=fig_size)
    
    # Choose color map based on the number of unique labels
    if len(np.unique(concat_labels)) > 10:
        colors = plt.get_cmap('tab20').colors
    else:
        colors = plt.get_cmap('tab10').colors

    # Plot each group
    unique_labels = np.unique(concat_labels)
    for i in range(len(unique_labels)):
        label = unique_labels[i]
        mask = np.array(concat_labels) == label
    ...
    if save_path:
        plt.savefig(save_path, transparent=True, format='svg', dpi=dpi_value, bbox_inches='tight')
    else:
        plt.show()


def plot_anchor_patch(adata, gene_name, cnt_color, 
                      fig_size=(2, 2), dpi_value=100):
    """
    Adjusts spatial coordinates and plots the data from an AnnData object.
    
    Parameters:
    adata : AnnData
        The AnnData object containing the spatial coordinates and gene expression data.
    gene_name : str
        The name of the gene to color code the scatter plot.
    """
    ## Re-order coords according to x-axis, and save the index
    sorted_indices = np.argsort(adata.obsm['spatial'][:, 0])
    points = adata.obsm['spatial'][sorted_indices]
    # print("Order-x coords: \n", points)

    columns = [points[i::3] for i in range(3)]
    columns
    ...
    for i, col in enumerate(columns):
        min_x_index = np.argmin(col[:, 0])
        base_y = col[min_x_index, 1]
        for idx in range(len(col)):
            adjusted_points[i + idx * 3, 1] = base_y
    # print("Step1: \n", adjusted_points)

    # 将调整后的点分为三行，每行三个点
    rows = [adjusted_points[i:i+3] for i in range(0, len(adjusted_points), 3)]
    rows

    # 对每一行进行处理，以调整x坐标
    for i, row in enumerate(rows):
        min_y_index = np.argmin(row[:, 1])
        base_x = row[min_y_index, 0]
        for idx in range(len(row)):
            adjusted_points[i * 3 + idx, 0] = base_x
    # print("Step2: \n", adjusted_points)

    # 使用获取的排序索引的逆操作，将调整后的点重新排序，使其顺序与原始数据一致
    adjusted_points = adjusted_points[np.argsort(sorted_indices)]

    adata_anchor_nebs_patch = adata.copy()
    adata_anchor_nebs_patch.obsm['spatial'] = adjusted_points
    print("Adjusted coords: \n", adata_anchor_nebs_patch.obsm['spatial'])
    print("Original coords: \n", adata.obsm['spatial'])

    fig, ax = plt.subplots(figsize=fig_size)
    plt.scatter(adata_anchor_nebs_patch.obsm['spatial'][:, 0], adata_anchor_nebs_patch.obsm['spatial'][:, 1], 
                c=adata_anchor_nebs_patch.to_df()[gene_name], s=1500, cmap=cnt_color)

    # 设置x轴和y轴的刻度
    x_ticks = np.unique(adata_anchor_nebs_patch.obsm['spatial'][:, 0])
    y_ticks = np.unique(adata_anchor_nebs_patch.obsm['spatial'][:, 1])
...
def spot_triangle(points_list, fig_size=4):
    fig, axs = plt.subplots(1, len(points_list), figsize=(fig_size*len(points_list), fig_size))
    
    ## plot triangle
    for i, ax in enumerate(axs):
        points = points_list[i]
        ## add one point to colse triangle
        triangle = np.vstack([points, points[0]])
        ax.plot(triangle[:, 0], triangle[:, 1], color='tab:blue', linestyle='-')    # 'b-', blue line
        ax.scatter(points[:, 0], points[:, 1], color='red')    # mark vertic
        for (x, y) in points:
            ax.text(x, y, f'({x}, {y})', color='blue', fontsize=12)
        ## Calculate and label the degree of each angle
        for j in range(3):
            p1 = points[j] - points[(j+1) % 3]
            p2 = points[j] - points[(j+2) % 3]
            angle = np.arccos(np.dot(p1, p2) / (np.linalg.norm(p1) * np.linalg.norm(p2)))
            angle = np.degrees(angle)
            ax.text(points[j, 0] + 30, points[j, 1] - 30, f'{angle:.2f}°', color='green', fontsize=12)
...
def plot_anchor_spots(adata_nebs, adata_anchor, gene_name, 
                      cnt_color, fig_size=(4, 2), dpi_value=100, show_anchor=True):
    """
    Plots spatial gene expression data from AnnData objects with optional highlighting of anchor points.
    
    Parameters:
    adata_nebs : AnnData
        The AnnData object containing the spatial coordinates and gene expression data for neighbor points.
    adata_anchor : AnnData
        The AnnData object containing the spatial coordinates and gene expression data for anchor points.
    gene_name : str
        The name of the gene to color code the scatter plot.
    include_anchor : bool, optional
        Flag to include scatter plot for anchor points. Defaults to True.
    """
    # Create the plot
    fig, ax = plt.subplots(figsize=fig_size)
    
    # Scatter plot for nearest neighbor points
    scatter1 = ax.scatter(adata_nebs.obsm['spatial'][:, 0], adata_nebs.obsm['spatial'][:, 1],
                          c=adata_nebs.to_df()[gene_name], s=1500, cmap=cnt_color)
    
    ## Set axes
    x_ticks = np.unique(adata_nebs.obsm['spatial'][:, 0])
    y_ticks = np.unique(adata_nebs.obsm['spatial'][:, 1])
    ax.set_xticks(x_ticks)
    ax.set_yticks(y_ticks)
    ax.set_xticklabels(x_ticks, rotation=60)
    ax.set_yticklabels(y_ticks)
    ...
    if show_anchor:
        # Optional: scatter plot for anchor points with a different color map
        scatter2 = ax.scatter(adata_anchor.obsm['spatial'][:, 0], adata_anchor.obsm['spatial'][:, 1],
                              c=adata_anchor.to_df()[gene_name], s=1500, cmap='Dark2')
    
    # Adding colorbar for the main scatter plot
